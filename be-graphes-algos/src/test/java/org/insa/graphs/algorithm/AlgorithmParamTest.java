...
public class AlgorithmParamTest extends AlgorithmTest {
    ...
    @Parameters(name = "{index}: Carte {0}, de {1} vers {2} au plus court {3} avec {4}")
    ...
    public void testDijkstraShortest(){
        double ecart = cheminPratique(graph, origine, destination, Mode.LENGTH, TypeAlgorithme.Dijkstra);
        if(ecart > PRECISION){
    ...
    public void testDijkstraFastest(){
        double ecart = cheminPratique(graph, origine, destination, Mode.TIME, TypeAlgorithme.Dijkstra);
        if(ecart > PRECISION){
            fail("Ecart de distance de plus de "+PRECISION+" avec "+ecart);
        }
    }
    ...
    public void testAStarShortest(){
        double ecart = cheminPratique(graph, origine, destination, Mode.LENGTH, TypeAlgorithme.AStar);
        if(ecart > PRECISION){
            fail("Ecart de distance de plus de "+PRECISION+" avec "+ecart);
        }
    }
    ...
    public void testAStarFastest(){
        double ecart = cheminPratique(graph, origine, destination, Mode.TIME, TypeAlgorithme.AStar);
        if(ecart > PRECISION){
            fail("Ecart de distance de plus de "+PRECISION+" avec "+ecart);
        }
    }
